...
async function measurePDFJsWithLongTasks(testUrl, versionName) {
  ...
  const browser = await puppeteer.launch({
    ...
    protocolTimeout: 300000, // 5분 - 현실적 패턴은 시간이 오래 걸림
  });

  const page = await browser.newPage();
  
  // 타임아웃 설정 (5분) - 현실적 패턴은 시간이 오래 걸림
...
(async () => {
  ...
  if (outputFile) {
    // 출력 파일이 지정된 경우
    outputPath = path.isAbsolute(outputFile) ? outputFile : path.join(outDir, outputFile);
  } else {
    // 타임스탬프가 포함된 새 파일 생성
  ...
  pdfVersions.forEach(pdfVer => {
    ...
    if (averages[queueVer]) {
      ...
      console.log(`\n📊 ${suffix} (PDF: n=${pdfAvg.count}, Queue: n=${queueAvg.count})`);
      console.log('-'.repeat(80));
