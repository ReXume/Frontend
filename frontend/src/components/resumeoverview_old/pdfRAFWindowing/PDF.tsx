...
const PDF = forwardRef<HTMLDivElement, PDFProps>(
  ({ pdf, pageNumber, addFeedbackPoint, feedbackPoints, hoveredCommentId }, ref) => {
    ...
    // 함수들을 ref로 관리하여 안정적인 참조 유지
    const renderPageRef = useRef<(() => Promise<void>) | null>(null);
    const resetPageRef = useRef<(() => void) | null>(null);

    // 페이지 크기 미리 얻어 placeholder 고정 (한 번만 실행)
    useEffect(() => {
      let cancelled = false;
      (async () => {
        try {
          ...
          console.error(`Failed to get page ${pageNumber}:`, e);
        }
      })();
      return () => {
        cancelled = true;
      };
    }, [pdf, pageNumber]); // pdf와 pageNumber만 의존성으로 설정
    ...
    // 메서드들을 DOM 요소에 부착하는 effect (의존성 없이 매번 업데이트)
    useEffect(() => {
      const el = hostRef.current;
      if (!el) return;

      (el as any).renderPage = async () => {
        const renderFn = renderPageRef.current;
        if (renderFn && !renderedRef.current) {
          await renderFn();
        }
      };
      (el as any).rendered = () => renderedRef.current;
      (el as any).resetPage = () => {
        const resetFn = resetPageRef.current;
        if (resetFn) resetFn();
      };
    }); // 의존성 배열 없음 - 매 렌더마다 최신 함수 참조로 업데이트
