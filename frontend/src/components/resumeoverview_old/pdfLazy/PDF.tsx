...
const PDF = forwardRef<HTMLDivElement, PDFProps>(
  ({ pdf, pageNumber, addFeedbackPoint, feedbackPoints, hoveredCommentId }, ref) => {
    ...
    // viewportWH 상태 제거 - 고정 비율 사용

    // 페이지 크기 미리 계산 Effect 제거됨 - 실제 렌더링 시에만 getPage() 호출

    // 외부에서 부르는 렌더 함수 - getPage() 호출이 여기로 이동
    const renderPage = useCallback(async () => {
      if (renderedRef.current || !canvasRef.current) return;
      try {
        const t0 = performance.now();
        // 실제 렌더링 시점에 getPage() 호출
    ...
    // 고정 비율 placeholder - A4 비율 (1/1.414)
    const placeholderStyle: React.CSSProperties = { 
      width: "100%", 
      aspectRatio: "1/1.414", // A4 비율 고정
      background: "#f3f4f6" 
    };
