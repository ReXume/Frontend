...
import { renderSchedulerFixed } from "@/libs/renderSchedulerFixed";
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        console.log(`[고정K=5] Page ${pageNumber} intersection:`, entry.isIntersecting);
        
        if (entry.isIntersecting) {
          // 이미 렌더링되었으면 스킵
          if (renderedRef.current) {
            console.log(`[고정K=5] Page ${pageNumber} already rendered`);
            return;
          }
          ...
          console.log(`[고정K=5] Enqueuing page ${pageNumber} with priority ${priority}`);
          
          renderSchedulerFixed.enqueue({
            id: jobId,
            priority,
            run: async () => {
              if (renderedRef.current) return;

              console.log(`[고정K=5] Starting render for page ${pageNumber}`);
              ...
              if (!canvas) {
                console.error(`[고정K=5] Canvas not found for page ${pageNumber}`);
                return;
              }
              
              const ctx = canvas.getContext("2d");
              if (!ctx) {
                console.error(`[고정K=5] Context not found for page ${pageNumber}`);
                return;
              }
              ...
              try {
                ...
                const metrics = {
                  ...
                  schedulerType: 'fixed-k5',
                };
                
                console.log(`[고정K=5] Page ${pageNumber} rendered - getPage: ${metrics.getPageMs}ms, render: ${metrics.renderMs}ms, paint: ${metrics.paintMs}ms, total: ${metrics.totalMs}ms`);
                ...
                if (e?.name === "RenderingCancelledException") {
                  console.log(`[고정K=5] Page ${pageNumber} rendering cancelled`);
                } else {
                  console.error(`[고정K=5] page ${pageNumber} render error`, e);
                }
              }
            },
            cancel: () => {
          ...
          // 화면에서 멀어지면 취소
          console.log(`[고정K=5] Page ${pageNumber} out of view, cancelling`);
          renderSchedulerFixed.cancel(jobId);
        }
      },
      {
        root: null,
