...
const initializeWorker = (): void => {
  ...
  try {
    console.log('[고정K=5] PDF 워커 초기화 시작');
    
    // public 폴더의 워커 파일 사용 (가장 안정적)
    GlobalWorkerOptions.workerSrc = '/pdf.worker.min.js';
    
    console.log('[고정K=5] PDF 워커 경로 설정 완료:', GlobalWorkerOptions.workerSrc);
    workerInitialized = true;
    
  } catch (error) {
    console.error('[고정K=5] PDF 워커 초기화 실패:', error);
    throw error;
  }
};
...
const PDFViewer = ({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        console.log(`[고정K=5] PDF 로딩 시작: ${pdfSrc}`);
        ...
        console.log('[고정K=5] 워커 초기화 완료, PDF 문서 로딩 시작');
        ...
        console.log(`[고정K=5] PDF 문서 로딩 완료: ${loaded.numPages}페이지`);
        setPdf(loaded);
        ...
        if (cancelled) return;
        console.error("[고정K=5] PDF 로딩 실패:", e);
